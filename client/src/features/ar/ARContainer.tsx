    ...
    <div className="relative min-h-[500px] w-full bg-slate-950 rounded-2xl overflow-hidden border border-amber-500/20 flex flex-col justify-between p-6">
      ...
      <div className="flex items-center justify-between bg-slate-900/80 backdrop-blur-md p-4 rounded-xl border border-slate-800">
        <div className="flex items-center space-x-3">
          ...
          <span className="font-semibold text-amber-200">AR Marker Scanner</span>
        </div>
        <button
          onClick={onBackTo3D}
      ...
      {/* Camera / AR Target Placeholder View */}
      <div className="my-8 flex flex-col items-center justify-center text-center p-8 bg-slate-900/40 rounded-xl border border-dashed border-amber-500/30">
        ...
        <h3 className="text-lg font-bold text-slate-100 mb-2">MindAR Image Target Scanner</h3>
        <p className="text-sm text-slate-400 max-w-md mb-6">
          Point your phone camera at the printed Taj Mahal target marker to project the 3D monument in augmented reality.
        </p>

        <div className="bg-amber-950/40 border border-amber-500/30 rounded-lg p-4 text-left max-w-md">
          <div className="flex items-center space-x-2 text-amber-400 text-xs font-semibold uppercase tracking-wider mb-1">
            <AlertTriangle className="w-4 h-4" />
            <span>Feasibility Test Mode</span>
          </div>
          <p className="text-xs text-amber-200/80">
            Siddhant is testing MindAR integration on <code className="text-amber-300 bg-amber-900/50 px-1 py-0.5 rounded">feature/siddhant-ar</code>. Camera permissions will be requested on supported mobile browsers.
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between text-xs text-slate-400 bg-slate-900/80 p-3 rounded-xl">
        <span>Status: Initialized (Waiting for Target)</span>
        <button 
          onClick={onBackTo3D}
